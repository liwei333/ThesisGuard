...
def build_completion(records: list[dict[str, Any]], invocation_id: str) -> dict[str, Any]:
...
def run_formal(
    *,
    ...
    marker = build_completion(records, invocation_id)
...
def _synthetic_record(invocation_id: str, sequence: int, digest: str) -> dict[str, Any]:
    return {
        "schema_version": SCHEMA_VERSION,
        "task_id": TASK_ID,
        "invocation_id": invocation_id,
...
def dry_run(root: Path) -> dict[str, Any]:
    ...
    digest = "d" * 64
    records = [_synthetic_record(invocation_id, index, digest) for index in range(7)]
    ...
    publish_completion_after_timeline_fsync(
        timeline, completion_path, build_completion(records, invocation_id)
    )
    atomic_publish_json(
        outcome_path,
        {
