"""WP-04-02 R4/R5 structured pytest collection plugin.
...
the R4/R5 runner consumes.  No terminal-output parsing, no ``<Coroutine ...>``
string matching, no dependency on the display format.

The plugin is loaded via ``-p tg_verifier_tools.verification.wp04_02_r4_pytest_plugin``.
...
- Atomic output: the JSON is written to a temporary file then renamed so a
  crash mid-write can never leave a partial file.
- Invocation nonce: the plugin reads ``TG_R4_INVOCATION_NONCE`` from the
  environment and includes it in the output so the runner can bind the
  collection result to the current invocation and reject stale JSON.
"""
...
EXPECTED_NODEIDS: tuple[str, ...] = (
    # 20 x forbidden_status_commit_fresh_no_residue  (4 routes x 5 denied statuses)
    ...
    # 16 x allowed_routing_history_audit  (4 routes x 2 statuses x 2 explicit_none)
    ...
    # 3 x committed_exact_replay_read_only
    ...
    # 2 x legacy_forbidden_prior_committed_replay
...
def compute_distribution(nodeids: list[str]) -> dict[str, int]:
    ...
    dist: dict[str, int] = dict.fromkeys(DISTRIBUTION, 0)
    for nid in nodeids:
...
def pytest_collection_finish(session: pytest.Session) -> None:
    ...
    The output is published atomically (write to temp, then rename) and carries
    the invocation nonce from ``TG_R4_INVOCATION_NONCE`` so the runner can bind
    the result to the current run and reject stale artefacts.
    """
    output_path: Path | None = getattr(session.config, "_r4_collection_output", None)
    if output_path is None:
        return

    nonce = os.environ.get("TG_R4_INVOCATION_NONCE", "")
